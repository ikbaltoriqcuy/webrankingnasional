...
<html>
<head>
<title>Validasi Data Nasional</title>
...
<span id="script"> 
	...
	<script>
		function callData() {			
				var validasiData = document.getElementById("selectTreg").value;
    			...
    			if (validasiData == "") {
					...
					xmlhttp.open("GET","queryDataValidasi.php?q="+validasiData+" "+month,true);
					xmlhttp.send();
				}
		}
	</script>
	...
	$.ajax({
		url: "http://localhost/tesexcel/jsonValdat.php?q="+month,
		method: "GET",
...
<div id="title" style="color:#fff;padding-top:15px;width:100%; height:100px;"> 
    <div id="text-title" style="margin-left:10%;margin-top:1%;"> 
		...
		<h4 class="sub_menu" style="padding:5px;text-align:center;float:left;margin-left:10px;width:200px;background-color:#ffa000;border-radius:10px;">Validasi Data</div> 
	</div>
</div>
...
<button class="btn btn-primary"  onclick="openFormUpload()"> Upload Data Excel </button>
	...
	<br/><br/>
	<select class="form-control" onchange="location = 'validasi.php?q=' + this.value" id="changeMonth">
		<?php
		$month = "";
		if(isset($_GET['q'])) { 
			$month =$_GET['q'];
...
<table id="contoh" class="table table-dark">
  <thead align="center">
   <tr>
        <th>RANK</th>
        <th>TREG
			<select id="selectTreg" style="color:#000;" class="selectpicker" onchange="callData()"> 
        ...
        <th>WITEL</th>
        <th>JUMLAH ODP UIM</th>
		<th>REAL ODP TERVALIDASI</th>
		<th>% REAL ODP TERVALIDASI</th>
		<th>% TARGET ODP TERVALIDASI</th>
		<th>% PENCAPAIAN</th>
   </tr>
 </thead>
 ...
 <?php
	 if(isset($_GET['q'])) {
		$query = mysqli_query($conn,"SELECT * FROM valdat WHERE bulan='".$_GET['q']."' ORDER BY ranking ASC");
	}else{
		$query = mysqli_query($conn,"SELECT * FROM valdat  ORDER BY ranking ASC");
	} 
	  $no = 1;
